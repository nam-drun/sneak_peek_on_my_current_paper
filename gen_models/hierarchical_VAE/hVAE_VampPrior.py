"""
Stochastic Interpolant (SI)–aligned notation for VAE + VampPrior training
========================================================================

We *use* x₀ ~ ρ₀ ⊂ 𝒳 = ℝᵈ (data endpoint in SI) synthesized by `toy3_si.py` (this file imports from it).
We *learn* a VAE with encoder q_φ(z|x), decoder p_θ(x|z), and the VampPrior (Eq. 12 in Tomczak & Welling 2018):

    p_λ(z) = (1/K) ∑_{k=1}^K q_φ(z | u_k)                                   [VampPrior, Eq. (12)]
...
LR_MIN = 1e-08
LR_MAX = 8e-05
WARMUP_STEPS = 261  #50% of the pretraining
...
# If you are ML Eng/Research Eng and need to run this outside of Google Colab: uncomment these lines
...
ENCODER_BLOCKS = 1      # number of MLP blocks in encoder backbone
DEC_BLOCKS     = 1      # number of MLP blocks in decoder backbone

d    = 56*56*3          # Height = 56, Width = 56, Channels = 3
...
def log_p_lambda_vamp(z: torch.Tensor, q_phi: Q_phi, U: torch.nn.Parameter) -> torch.Tensor:
    """
    VampPrior log-density: log p_λ(z) = log (1/K ∑_k q_φ(z | u_k))
    where q_φ(z|u_k) is Gaussian from encoder evaluated at pseudo-input u_k.
    """
    mu_u, logvar_u = q_phi(U)   # (K, Z)
    z_b = z.unsqueeze(1)        # (B, 1, Z)
    # pairwise log q for all K components
    lp_mat = log_normal_diag(z_b, mu_u.unsqueeze(0), logvar_u.unsqueeze(0))  # (B, K)
    return torch.logsumexp(lp_mat - math.log(U.size(0)), dim=1)              # (B,)
...
# Training loops
# -----------------
def train_unsupervised_VampPrior(*, epochs=5, batch=384, Z_DIM=32, H=512, K_vamp=50, total_samples=25000, shuffle=True):
    q_phi = Q_phi(d, Z_DIM, h=H).to(DEVICE)
    Tdec  = T_theta(Z_DIM, d, h=H).to(DEVICE)
    U     = torch.nn.Parameter(torch.randn(K_vamp, d, device=DEVICE) * 0.1)  # pseudo-inputs
    opt   = torch.optim.AdamW(list(q_phi.parameters()) + list(Tdec.parameters()) + [U], lr=LR_MAX)
    ...
    for ep in range(1, epochs+1):
        ...
        for (x_b,) in loader:
            ...
            log_p = log_p_lambda_vamp(z, q_phi, U)
    ...
    return dict(q_phi=q_phi, Tdec=Tdec, U=U, loss_history=loss_hist, lr_history=lr_history)


def train_unsup_CFG_VampPrior(*, epochs=5, batch=384, Z_DIM=32, H=512, K_vamp=50, p_drop=0.2, total_samples=25000, shuffle=True):
    q_phi = Q_phi(d, Z_DIM, h=H).to(DEVICE)
    TdecC = T_theta_cond(Z_DIM, d, n_classes=3, cond_dim=32, h=H).to(DEVICE)
    U     = torch.nn.Parameter(torch.randn(K_vamp, d, device=DEVICE) * 0.1)
    opt   = torch.optim.AdamW(list(q_phi.parameters()) + list(TdecC.parameters()) + [U], lr=LR_MAX)
    ...
    return dict(q_phi=q_phi, TdecC=TdecC, U=U, loss_history=loss_hist, lr_history=lr_history)

# -----------------
# Sampling helpers
# -----------------
@torch.no_grad()
def sample_prior_Vamp(U: torch.nn.Parameter, q_phi: Q_phi, n: int, d_z: int) -> torch.Tensor:
    """
    Approximate ancestral sampling from p_λ(z) by:
      1) Pick component k ~ Uniform{1..K}
      2) Compute q_φ(z|u_k) params and draw one z
    """
    K = U.size(0)
    idx = torch.randint(0, K, (n,), generator=GEN, device=DEVICE)
    # evaluate encoder on those u_k
    mu_all, logvar_all = q_phi(U)         # (K, Z)

    # engineering-ish lined-of-code because original VAE + VampPrior has lots of numerical instability
    logvar_all = logvar_all.clamp(min=-10.0, max=10.0)

    mu, logvar = mu_all[idx], logvar_all[idx]
    eps = torch.randn(n, d_z, device=DEVICE, generator=GEN)
    return mu + eps * torch.exp(0.5 * logvar)

@torch.no_grad()
def generate_unsup(art, n=4000, Z_DIM=32):
    z = sample_prior_Vamp(art["U"], art["q_phi"], n=n, d_z=Z_DIM)
    xhat = art["Tdec"](z)
...
if __name__ == "__main__":
    ...
    art_unsup = train_unsupervised_VampPrior(epochs=EPOCHS, batch=384, Z_DIM=32, H=512, K_vamp=K_vamp, total_samples=total_samples)
    art_cfg   = train_unsup_CFG_VampPrior(epochs=EPOCHS, batch=384, Z_DIM=32, H=512, K_vamp=K_vamp, p_drop=0.2, total_samples=total_samples)
