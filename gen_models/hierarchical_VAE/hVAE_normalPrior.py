"""
Stochastic Interpolant (SI)–aligned notation for VAE + Standard Gaussian (SG) prior training
=============================================================================================

We *use* x₀ ~ ρ₀ ⊂ 𝒳 = ℝᵈ (data endpoint in SI) synthesized by `toy3_si.py` (this file imports from it).
We *learn* a VAE with encoder q_φ(z|x), decoder p_θ(x|z), and the **standard Gaussian prior**:

    p_λ(z) = N(z; 0, I)                                                            [SG prior, "Alternative priors"]
...
- Symbols (kept consistent with SI frame):
    d                             := ambient dimension
...
This file mirrors the structure of the VampPrior and MoG variants, but uses the SG prior p_λ(z)=N(0,I).
"""
...
# -----------------------
# LR schedule (warmup + linear decay)
# -----------------------
LR_MIN = 1e-08
LR_MAX = 8e-05
WARMUP_STEPS = 261
...
class SwiGLU(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        chunks = x.split(self.hidden, dim=-1)
...
def log_p_lambda_gaussian(z: torch.Tensor) -> torch.Tensor:
    """Standard Gaussian prior log-density: log N(z; 0, I). Returns (B,)."""
    B, Z = z.shape
    return -0.5 * (z.pow(2).sum(dim=1) + Z * math.log(2*math.pi))
...
# -----------------
# Training loops (SG prior)
# -----------------
def train_unsupervised_SG(*, epochs=5, batch=384, Z_DIM=32, H=512, total_samples=25000, lr: float = LR_MAX, shuffle=True):
    q_phi = Q_phi(d, Z_DIM, h=H).to(DEVICE)
    Tdec  = T_theta(Z_DIM, d, h=H).to(DEVICE)
    opt   = torch.optim.AdamW(list(q_phi.parameters()) + list(Tdec.parameters()), lr=lr)
    ...
    return dict(q_phi=q_phi, Tdec=Tdec, loss_history=loss_hist, lr_history=lr_history)


def train_unsup_CFG_SG(*, epochs=5, batch=384, Z_DIM=32, H=512, p_drop=0.2, total_samples=25000, lr: float = LR_MAX, shuffle=True):
    q_phi = Q_phi(d, Z_DIM, h=H).to(DEVICE)
    TdecC = T_theta_cond(Z_DIM, d, n_classes=3, cond_dim=32, h=H).to(DEVICE)
    opt   = torch.optim.AdamW(list(q_phi.parameters()) + list(TdecC.parameters()), lr=lr)
    ...
    return dict(q_phi=q_phi, TdecC=TdecC, loss_history=loss_hist, lr_history=lr_history)
...
# -----------------
@torch.no_grad()
def sample_prior_SG(n: int, d_z: int, generator=None) -> torch.Tensor:
    """Draw z ~ N(0, I)."""
    if generator is None:
        generator = GEN if 'GEN' in globals() else None
    return torch.randn(n, d_z, device=DEVICE, generator=generator)

@torch.no_grad()
def generate_unsup(art, n=4000, Z_DIM=32):
    z = sample_prior_SG(n, Z_DIM)
    xhat = art['Tdec'](z)
...
if __name__ == "__main__":
    ...
    art_unsup = train_unsupervised_SG(epochs=EPOCHS, batch=384, Z_DIM=32, H=512, total_samples=total_samples, lr=LR_MAX)

    art_cfg   = train_unsup_CFG_SG(epochs=EPOCHS, batch=384, Z_DIM=32, H=512, lr=LR_MAX, p_drop=0.2, total_samples=total_samples)
    ...
    plt.scatter(Y_gen_u[:,0], Y_gen_u[:,1], s=2, alpha=0.55, c="tab:purple", label="VAE+Gaussian (gen)")
